...
export const CreateBookmarkSchema = z.object({
  url: z.string().url().max(2048).openapi({ example: 'https://example.com' }), // 実用的な上限: RFC7230推奨8000文字、Safari視認性限界4096文字、実用推奨2000文字前後
  title: z.string().min(1).max(200).optional().openapi({ example: 'Example Website' }),
});

export const UpdateBookmarkSchema = z.object({
  url: z.string().url().max(2048).optional(), // 実用的な上限: RFC7230推奨8000文字、Safari視認性限界4096文字、実用推奨2000文字前後
